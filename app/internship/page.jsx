...
export default function InternshipPage() {
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      ...
      <AudioRecorder onCommitted={() => setReloadKey((k) => k + 1)} />
      <Notes reloadKey={reloadKey} />
    </div>
  );
}
...
function AudioRecorder({ onCommitted }) {
  ...
  async function start() {
    ...
    try {
      ...
      rec.onstop = () => {
        ...
        setClips((c) => [{ id: stamp() + "-" + Math.random().toString(36).slice(2, 6), url, blob, name: `internship-audio-${stamp()}.${extFor(type)}`, size: blob.size, committed: false, busy: false }, ...c]);
      };
  ...
  async function commit(clip) {
    ...
    try {
      ...
      const r = await fetch("/api/internship/recording", { method: "POST", body: fd });
      if (!r.ok) throw new Error("upload failed");
      const { relPath, fileName } = await r.json();
      await store.add("internship_notes", { text: "", recording: relPath, recordingName: fileName, ts: new Date().toISOString() });
...
function Notes({ reloadKey }) {
  const [notes, setNotes] = useState([]);
  const [text, setText] = useState("");
  const [saving, setSaving] = useState(false);

  async function load() { try { setNotes(await store.list("internship_notes")); } catch {} }
  ...
  async function add() {
    ...
    setSaving(true);
    try {
      await store.add("internship_notes", { text: text.trim(), ts: new Date().toISOString() });
      setText(""); load();
    } finally { setSaving(false); }
  }
  async function saveText(id, value) { await store.update("internship_notes", id, { text: value }); }
  async function del(id) { const { items } = await store.remove("internship_notes", id); setNotes(items); }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 11, color: "var(--color-muted)" }}>Mirrored to <code>20 - Areas/Internship/Internship Notes.md</code></span>
      </div>

      <div className="card" style={{ padding: 16, marginBottom: 18 }}>
        <textarea className="input" rows={3} placeholder="Add a note (meeting summary, follow-ups, project updates)…" value={text}
